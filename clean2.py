...
def extract_district(location):
    ...
    # Regular expression to match "Quận/Huyện XXX" pattern
    district_match = re.search(r'(Quận|Huyện|Thị Xã)\s+([^,]+)', location)
    if district_match:
        district_type = district_match.group(1)  # Quận or Huyện
        district_name = district_match.group(2).strip()  # District name
        return f"{district_type} {district_name}"
    return None
